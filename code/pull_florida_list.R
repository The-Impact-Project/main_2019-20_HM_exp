...
raw_fl_dat <- "SELECT 
  ...
  voterbase_email
FROM ts.ntl_current
  LEFT JOIN impactproject.fl_persuasion_scores USING (vb_voterbase_id)
...
WHERE ts_tsmart_presidential_general_turnout_score > 40
  AND vb_tsmart_state = 'FL'
  AND vb_tsmart_hd IN ('058', '060', '065', '067', '115', '116', '118', '119')
...
raw_fl_dat %>%
  ...
  saveRDS(here("output", "raw_florida_dat.Rds"))

raw_fl_dat <- readRDS(here("output", "raw_florida_dat.RDS"))

florida_dat <- raw_fl_dat %>%
