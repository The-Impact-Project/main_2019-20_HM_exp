...
WHERE ts_tsmart_presidential_general_turnout_score > 40
  AND vb_tsmart_state = 'NV'
  AND ( vb_tsmart_hd IN ('004', '027', '029', '037') OR vb_tsmart_sd IN ('006', '008', '009'))
...
raw_nv_dat <- readRDS(here("output", "raw_nevada_dat.RDS"))

nevada_dat <- raw_nv_dat %>%
...
screen_dat <- read_csv(here("data", "TMC-NV-Screen.csv")) %>%
...
nevada_dat <- nevada_dat %>%
  mutate(HH_in_exp = if_else(vb_voterbase_phone %in% nevada_dat$vb_voterbase_phone[nevada_dat$in_experiment==1], 1, 0)) %>% # doesn't share Address or phone with exp
  mutate(phone_in_exp = if_else(HHID %in% nevada_dat$HHID[nevada_dat$in_experiment==1], 1, 0)) %>% # doesn't share phone with exp
  mutate(in_exp_HH_or_phone = if_else(HH_in_exp==1, 1, if_else(phone_in_exp==1, 1, 0)))

table(nevada_dat$combined_districts, nevada_dat$in_experiment)

proportion_in_treatment <- .45

randomized_dat <- nevada_dat %>%
  filter(in_experiment==1) %>%
  balance_randomization(block_vars = c("combined_districts"), 
                        cluster_vars = "HHID",
                        n_treatment = 2,
...
randomized_dat %>% 
  select(assignment, combined_districts) %>%
  table()

district_target_counts <- data.frame(combined_districts = c('hd_27','hd_29','hd_37','hd_4','sd_6','sd_8','sd_9'),
                                     target_goal = c(5000, 5000, 5000, 5000, 5000, 9500, 9500))

# see how many more we need in each District
counts_df <- randomized_dat %>%
  filter(assignment == "treatment") %>%
  group_by(combined_districts) %>%
  summarise(targeted_in_experiment_so_far = n()) %>%
  ungroup() %>%
  full_join(district_target_counts, by = "combined_districts") %>%
  mutate(more_needed = target_goal - targeted_in_experiment_so_far) %>%
  mutate(more_needed = if_else(more_needed > 0, more_needed, 0)) %>%
  full_join(filter(nevada_dat, in_exp_HH_or_phone==0) %>% 
              group_by(combined_districts) %>% 
              summarise(number_available_to_add = n()), 
            by = "combined_districts") %>%
...
randomized_dat <- nevada_dat %>%
  ...
  nest() %>%            
  ungroup() %>% 
  arrange(combined_districts) %>%
  left_join(counts_df, by = "combined_districts") %>%
...
randomized_dat <- randomized_dat %>%
  ...
  group_by(combined_districts, get_treated) %>%
  mutate(id = row_number()) %>%
  ungroup() %>%
  filter(id <=9500 | get_treated == FALSE) %>%
...
randomized_dat %>% 
  ...
  TIPtools::audience_document(output_directory = here("output", "audience_reports"), 
                              output_title = "Nevada sd HM Audience",
                              refresh_list = FALSE,
                              district_cross = "sd")
