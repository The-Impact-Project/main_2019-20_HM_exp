...
WHERE ts_tsmart_presidential_general_turnout_score > 40
  AND vb_tsmart_state = 'PA'
  AND vb_tsmart_hd IN ('029', '094', '100', '105', '131', '144', '168', '178')
...
raw_pa_dat %>%
  ...
  saveRDS(here("output", "raw_pennsylvania_dat.Rds"))

raw_pa_dat <- readRDS(here("output", "raw_pennsylvania_dat.Rds"))

pennsylvania_dat <- raw_pa_dat %>%
