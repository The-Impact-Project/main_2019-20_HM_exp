...
feature_train <- mod_df %>% 
  select(-c(assignment, Qfav_flipped_continous, Qfav))
w_train <- if_else(mod_df$assignment == "control", 0, 1)
yobs_train <- mod_df$Qfav_flipped_continous
...
```{r predictions}
cate_esti_rf <- EstimateCate(xl_rf, select(mod_df, -c(assignment, Qfav_flipped_continous, Qfav)))

mod_df$cate <- cate_esti_rf

mod_df %>%
  mutate(cate_cat = cut(cate, breaks = quantile(cate, probs = c(0,.3, .6, 1)), include.lowest = T)) %>%
