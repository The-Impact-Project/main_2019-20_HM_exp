...
To further examine our effect on unfavorable ratings in Republican held districts, the following plot shows the effect broken down by state. The results in Florida and Arizona are both significant on their own. The remaining states did not show significant changes.

```{r unfav_on_r_by_state, echo = FALSE, echo=FALSE, fig.height=3.5}
combined_weighted %>%
  filter(dem==0) %>%
  group_by(state, assignment, Q3) %>%
  summarize(n = sum(weight)) %>%
  mutate(freq = n / sum(n)) %>%
  filter(Q3==2) %>%
  select(-c(n, Q3)) %>%
  spread(assignment, freq) %>%
  mutate(unfav_effect = treatment - control) %>%
  ggplot(aes(x=reorder(state, -unfav_effect), y = unfav_effect)) +
    geom_bar(stat = "identity", fill = "#4dac26") +
    theme_minimal() +
    scale_y_continuous(labels = scales::percent_format(accuracy=1)) +
    labs(x = "", y = "Effect on Unfavorable Ratings (pp)")
```
