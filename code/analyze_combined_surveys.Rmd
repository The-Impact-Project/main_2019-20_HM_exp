...
cate_graph_effect <- (high_treatment - high_control) - (low_treatment - low_control)
cate_graph_effect <- round(cate_graph_effect*100, digits = 1)
```

The primary way of evaluating the model is measuring the effect (the difference between the control and treatment group), broken down by people with high and low model scores. The following graph shows this where the 25% of people with the highest CATE scores are classified as "High," and everyone else is classified as "Low." They important part of the graph is comparing the gap between the bars in the low group compared to that gap in the high group. It seems that the program had an effect in both groups, but it was far stronger in those with high CATE scores. The effect in the high group was `r cate_graph_effect` percentage points higher than the effect in the low group. This difference can be considered the "boost" in program effectiveness from using the score.
...
The graph above uses the 75th percentile as a somewhat arbitrary cutoff. Instead, we can examine what the boost in program effectiveness would be for using _any_ percentile cutoff. The following graph shows that for the CATE scores as well as the New Middle Scores and the TicketSplitter scores. 

The lines get more unstable, and less accurate towards the left and right sides of the graph, because in those cases either the "high" or the "low" group as a very small sample size. However towards the middle of the chart, there is a fairly clear trend showing the CATE model outperforming TicketSplitter, while the New Middle model hovers around zero indicating that it does not help at all.
...
score_effect_df %>%
  ...
  geom_line(size=1) +
  theme_minimal() +
  labs(y = "Boost From Using Score (pp)",
